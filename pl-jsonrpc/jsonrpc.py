...
def make_jsonrpc_object():
    return {"jsonrpc": "2.0"}


def make_jsonrpc_request(method, params, request_id=None):
    request = make_jsonrpc_object()
...
def make_jsonrpc_error(error_no, object_id=None):
    ret = make_jsonrpc_object()
    ret['error'] = jsonrpc_errors[str(error_no)]
    ret['id'] = object_id
    return ret


def make_jsonrpc_result(result, object_id):
    ret = make_jsonrpc_object()
    ret['result'] = result
    ret['id'] = object_id
    return ret
...
def call_jsonrpc(methods, request_json):
    try:
        request_object = json.loads(request_json)
    except Exception:
        return make_jsonrpc_error(-32700)
    ...
    if method is None:
        return make_jsonrpc_error(-32601, request_object['id'])
    ...
    try:
        ...
        return make_jsonrpc_error(-32603, request_object['id'])

    return make_jsonrpc_result(result, object_id)
